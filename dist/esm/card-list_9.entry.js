...
const FilterHeaderBar = class {
    ...
    getViews() {
        ...
        for (let viewType in this.views) {
            ...
            views.push(h("div", { class: `filter-label-wrap ${viewType}` }, h("span", { class: `filter-label-icon ${viewType} ${(this.activeView == viewType) ? 'active' : ''}` }), h("div", { class: `filter-label-type ${viewType} ${(this.activeView == viewType) ? 'active' : ''}`, onClick: () => this.handleView(viewType) }, viewName)));
        }
        return views;
    }
    handleView(viewType) {
...
const NoUiSliderWrapper = class {
    ...
    render() {
        return (h(Host, null, h("slot", { name: "title" }), h("div", { ref: el => this.el = el, id: "double-slider" })));
    }
    static get watchers() { return {
        "start": ["watchStart"]
    }; }
...
const PropertyCard = class {
    ...
    render() {
        const Card = (h("div", { class: `property-card${(this.postData.ID == this.activePostId) ? ' active' : ''}` }, h("div", { class: "property-card-content-wrap" }, h("div", { class: "property-image-wrap" }, h("img", { src: this.postData.thumbnail })), h("div", { class: "property-content" }, h("div", { class: "property-content-wrap" }, h("span", { class: "property-title" }, h("div", { class: "property-address" }, this.getPostTitle(), ","), " ", this.postData.meta.city[0], " ", this.postData.meta.state_code[0], " ", this.postData.meta.zip_code[0]), h("p", null, h("span", null, "Region:"), " ", this.postData.meta.region[0]), h("p", null, h("span", null, "Submarket:"), " ", this.postData.meta.sub_market[0]), h("p", null, h("span", null, "Total SQ FT:"), " ", this.postData.meta.sq_ft[0]), h("a", { class: "post-link", href: this.postData.link }, "View Details"))))));
        if (this.view == 'grid') {
...
const PropertyFilters = class {
    ...
    render() {
        return [
            h("div", { class: `property-filters${(this.modal) ? ' modal-state' : ''}` }, h("span", { class: "property-count-wrap" }, h("div", { class: "result-header" }, "Property Results"), h("span", { class: "property-count" }, this.posts.length), " properties match your results"), (this.modal) && (h("span", { class: "filter-title" }, "Filter")), h("input", { onChange: (e) => this.handleSearch(e), type: "text", value: (this.filters && this.filters.search) ? this.filters.search : '', placeholder: "Search properties by address", class: "search" }), h("select", { name: "regions", class: "dropdown", onChange: (e) => this.handleRegion(e) }, h("option", { selected: (this.filters && this.filters.region) ? false : true, disabled: true }, "Regions"), this.regions.map(region => h("option", { value: region.meta_value }, region.meta_value))), h("no-ui-slider-wrapper", { start: (this.filters && this.filters.sqFootage) ? this.filters.sqFootage : [0, 600], callback: this.handleSqFeet.bind(this) }, h("slot", { name: "title" }, "Square Footage")), h("select", { name: "sortby", class: "dropdown", onChange: (e) => this.handleSortBy(e) }, h("option", { selected: (this.filters && this.filters.sortBy) ? false : true, disabled: true }, "SortBy"), this.getSortBy()), h("button", { onClick: () => { (this.modal) ? this.modal = !this.modal : this.handleResetFilters(); }, class: "reset-button" }, (this.modal) ? 'Apply' : 'Reset Filters'), h("div", { class: "modal-close-button", onClick: () => { this.modal = !this.modal; } }, "X")),
