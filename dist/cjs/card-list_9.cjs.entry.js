...
const sortBy$1 = (data) => async (dispatch, _getState) => {
    return dispatch({
...
const FilterHeaderBar = class {
    ...
    getViews() {
        ...
        for (let viewType in this.views) {
            ...
            views.push(core.h("div", { class: `filter-label-wrap ${viewType}` }, core.h("span", { class: `filter-label-icon ${viewType} ${(this.activeView == viewType) ? 'active' : ''}` }), core.h("div", { class: `filter-label-type ${viewType} ${(this.activeView == viewType) ? 'active' : ''}`, onClick: () => this.handleView(viewType) }, viewName)));
        }
        return views;
    }
    handleView(viewType) {
...
const GoogleMap = class {
    ...
    findLatitudeAvg() {
        ...
        let lng = mean(map((post) => post.meta.longitude[0], this.posts));
        return {
            "lat": lat,
            "lng": lng
        };
...
const PropertyFilters = class {
    ...
    async getRegions() {
        let response = await axios$1.get(this.baseUrl + '/wp-json/bixby/v1/properties/regions');
    ...
    getSortBy() {
        ...
        for (let key in this.sortByInfo) {
            options.push((core.h("option", { value: key }, this.sortByInfo[key])));
        }
        return options;
    }
    handleSortBy(e) {
